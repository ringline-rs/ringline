---
name: pr
description: Create a feature branch, commit changes, push, and open a PR against ringline-rs/ringline
---
...
6. **Open PR against upstream**:
   - The upstream repo is `ringline-rs/ringline`
   - The fork remote is `origin` (determine the owner from `git remote -v`)
   - Use `gh pr create` with `--repo ringline-rs/ringline` and `--head <fork-owner>:<branch-name>`

   ```bash
   gh pr create \
     --repo ringline-rs/ringline \
...
- The fork owner can be determined from the origin remote URL (e.g., `https://github.com/brayniac/ringline` → `brayniac`)
