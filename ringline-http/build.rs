...
// the io_uring-only zero-copy recv path (the HTTP/2 pump feeds the h2 codec via
// `ConnCtx::with_segments` — a `#[cfg(has_io_uring)]` method — to drop the
// redundant `RecvAccumulator` copy).
//
